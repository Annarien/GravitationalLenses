...
max_num_testing = 1000
image_shape = (100, 100, 3)


# Helper methods
def getPositiveImages(max_num, input_shape):
    for root, dirs, _ in os.walk('Training/Positive'):
...
def getNegativeImages(max_num, input_shape):
    for root, dirs, _ in os.walk('Training/Negative'):
        num_of_images = min(max_num, len(dirs))
...
train_pos = getPositiveImages(max_num_training, input_shape=image_shape)

# Get positive testing data
test_pos = getPositiveImages(max_num_testing, input_shape=image_shape)

# Get negative training data
train_neg = getNegativeImages(max_num_training, input_shape=image_shape)

# Get negative testing data
test_neg = getNegativeImages(max_num_testing, input_shape=image_shape)
...
# Step 1 - Convolution
classifier.add(Conv2D(32, (3, 3), padding='same', input_shape=image_shape, activation='relu'))
...
classifier.save('galaxies_cnn.h5')
...
# Plot original image
img_tensor = getPositiveImages(1, input_shape=image_shape)
plt.figure()
...
classes = classifier.predict_classes(img_tensor, batch_size=10)
