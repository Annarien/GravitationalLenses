...
if not os.path.exists('../Results/g_r_%s/' % dt_string):
...
def gettingKFoldConfusionMatrix(test_data, test_labels, images_47, labels_47, images_84, labels_84, all_unseen_images,
    ...
    confusion_matrix_47 = gettingTrueFalsePositiveNegatives(images_47,
                                                            labels_47,
                                                            text_file_path='../Results/%s/Predicted47'
    ...
    confusion_matrix_84 = gettingTrueFalsePositiveNegatives(images_84,
                                                            labels_84,
                                                            text_file_path='../Results/%s/Predicted84'
    ...
    all_confusion_matrix = gettingTrueFalsePositiveNegatives(all_unseen_images,
                                                             all_unseen_labels,
                                                             text_file_path='../Results/%s/All_Predicted'
...
train_pos = getPositiveImages('Training/g_r_PositiveAll', max_num, input_shape=input_shape)
...
plt.ylabel("Accuracy")
# plt.show()
train_val_accuracy_figure.savefig('../Results/g_r_%s/TrainingValidationAccuracy.png' % dt_string)
plt.close()
...
plt.ylabel("Loss")
# plt.show()
train_val_loss_figure.savefig('../Results/g_r_%s/TrainingValidationLoss.png' % dt_string)
plt.close()

# make positive and negative directory
if not os.path.exists('../Results/g_r_%s/PositiveResults/' % dt_string):
    os.mkdir('../Results/g_r_%s/PositiveResults/' % dt_string)

if not os.path.exists('../Results/g_r_%s/NegativeResults/' % dt_string):
    os.mkdir('../Results/g_r_%s/NegativeResults/' % dt_string)

# Plot original positive image
img_positive_tensor = getPositiveImages('Training/g_r_PositiveAll', 1, input_shape=input_shape)
positive_train_figure = plt.figure()
plt.imshow(img_positive_tensor[0])
# plt.show()
print(img_positive_tensor.shape)
positive_train_figure.savefig('../Results/g_r_%s/PositiveResults/PositiveTrainingFigure.png' % dt_string)
plt.close()

# Visualise Activations of positive image
visualiseActivations(img_positive_tensor, base_dir='../Results/g_r_%s/PositiveResults/' % dt_string)
...
negative_train_figure.savefig('../Results/g_r_%s/NegativeResults/NegativeTrainingFigure.png' % dt_string)
plt.close()

# Visualise Activations of negative image
visualiseActivations(img_negative_tensor, base_dir='../Results/g_r_%s/NegativeResults/' % dt_string)
...
gettingTrueFalsePositiveNegatives(images_47,
                                  labels_47,
                                  text_file_path='../Results/g_r_%s/Predicted47/47_LensesPredicted.txt' % dt_string,
                                  predicted_lenses_filepath='../Results/g_r_%s/Predicted47' % dt_string)
...
gettingTrueFalsePositiveNegatives(images_84,
                                  labels_84,
                                  text_file_path='../Results/g_r_%s/Predicted84/84_LensesPredicted.txt' % dt_string,
                                  predicted_lenses_filepath='../Results/g_r_%s/Predicted84' % dt_string)
...
gettingTrueFalsePositiveNegatives(all_unseen_images,
                                  all_unseen_labels,
                                  text_file_path='../Results/g_r_%s/PredictedAll/All_LensesPredicted.txt' % dt_string,
                                  predicted_lenses_filepath='../Results/g_r_%s/PredictedAll' % dt_string)

savePredictedLenses(all_des_names,
                    all_predicted_class_probabilities,
                    predicted_lenses_filepath='../Results/g_r_%s/PredictedAll' % dt_string,
                    text_file_path='../Results/g_r_%s/Predicted84/All_LensesPredicted.txt' % dt_string)
