...
def makeModelImage(ml, rl, ql, b, ms, xs, ys, qs, ps, rs, num, survey = "DESc"):
    ...
    # Write FITS images
    outDir = "NewLenses/PositiveSimulated1000"
...
def addSky(num):
    ...
    if os.path.exists('NewLenses/PositiveWithDESSky/%i' % (num)) == False:
        os.makedirs('NewLenses/PositiveWithDESSky/%i' % (num))

    for band in ['g', 'r', 'i']:
        bandSkyImage = fits.open('NewLenses/DESSkyLast1000/%i_%s_sky.fits' % (num, band))
        bandPosNoiselessImage = fits.open('NewLenses/PositiveSimulated1000/%s/%s_image_%s_SDSS.fits' % (num, num, band))
        withSky = bandSkyImage[0].data + bandPosNoiselessImage[0].data
        astImages.saveFITS('NewLenses/PositiveWithDESSky/%i/%i_posSky_%s.fits' % (num, num, band), withSky)
       
def normalise(num, base_dir = 'NewLenses/PositiveWithDESSky'):
    """
    ...
    astPlots.ImagePlot([rgbDict['i'], rgbDict['r'], rgbDict['g']],
                        ...
                        axesFontSize= 26.0,
                        axes = [0, 0, 1, 1])
    plt.savefig('%s/%i/%i_rgb.png' % ('NewLenses/PositiveWithDESSky', num, num))
