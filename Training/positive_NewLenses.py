...
def makeModelImage(ml, rl, ql, b, ms, xs, ys, qs, ps, rs, num, survey="DESc"):
    ...
    S.setLensPars(ml, rlDict, ql, reset=True) # by removing the source, we can only look at the lens
    ...
    # Write FITS images
    outDir = "NewLenses/PositiveNoiseless"
...
def addLenseToDES(num,base_dir = 'NewLenses/LenseWithDES'):

    if os.path.exists(base_dir) == False:
        os.makedirs(base_dir)

    if os.path.exists('%s/%i/' % (base_dir, num)) == False:
        os.makedirs('%s/%i/' % (base_dir, num))

    size = 100, 100

    desPathName = 'DES/DES_Processed'
    desPaths = {}
    desPaths['iDesImg'] = glob.glob('%s/%s_*/i_WCSClipped.fits' % (desPathName, num))[0]
    desPaths['rDesImg'] = glob.glob('%s/%s_*/r_WCSClipped.fits' % (desPathName, num))[0]
    desPaths['gDesImg'] = glob.glob('%s/%s_*/g_WCSClipped.fits' % (desPathName, num))[0]

    lensPathName = 'NewLenses/PositiveNoiseless'
    lensPaths = {}
    lensPaths['iLensImg'] = glob.glob('%s/%s/%i_image_i_*.fits' % (lensPathName, num, num))[0]
    lensPaths['rLensImg'] = glob.glob('%s/%s/%i_image_r_*.fits' % (lensPathName, num, num))[0]
    lensPaths['gLensImg'] = glob.glob('%s/%s/%i_image_g_*.fits' % (lensPathName, num, num))[0]

    for band in ['g', 'r', 'i']:
            with fits.open(desPaths[band + 'DesImg']) as des:
                desImage = des[0].data
                desSquare = np.resize(desImage, size)
              
            with fits.open(lensPaths[band+'LensImg']) as lens:
                lensImage = lens[0].data
                lensSquare = np.resize(lensImage,size)
            
            combineImg = desSquare + lensSquare
            astImages.saveFITS('%s/%i/%i_%s_combined.fits' % (base_dir, num, num, band), combineImg)

def normalise(num, base_dir='NewLenses/LenseWithDES'):
    """
    ...
    """
    paths = {}
    paths['iImg'] = glob.glob('%s/%s/%s_i_combined.fits' % (base_dir, num, num))[0]
    paths['rImg'] = glob.glob('%s/%s/%s_r_combined.fits' % (base_dir, num, num))[0]
    paths['gImg'] = glob.glob('%s/%s/%s_g_combined.fits' % (base_dir, num, num))[0]
    ...
    astPlots.ImagePlot([rgbDict['i'], rgbDict['r'], rgbDict['g']],
                       ...
                       axesFontSize=26.0,
                       axes=[0, 0, 1, 1])
    plt.savefig('%s/%i/%i_rgb.png' % (base_dir, num, num))
...
for num in range(numStart, numEnd):
    ...
    g_ml = (lensRandomTable['Gmag'][rndmRow])   # ml in g band
    r_ml = (lensRandomTable['Rmag'][rndmRow])   # ml in r band
    i_ml = (lensRandomTable['Imag'][rndmRow])   # ml in i band
